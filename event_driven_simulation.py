...
class SimulacionLineaProduccion:
    def __init__(self,
        ...
        self.stats = {
            ...
            'bloqueos_fuente_tiempo': [], # NUEVO: lista de (tiempo, total perdidos)
            'estado_maquinas_tiempo': [], # NUEVO: [(tiempo, estado_m1, estado_m2, estado_m3)]
            'niveles_colas_tiempo': [],   # NUEVO: [(tiempo, len(cola1), len(cola2), len(cola3))]
        }
    ...
    def _registrar_estados(self):
        ...
        self.stats['estado_maquinas_tiempo'].append(
            (self.reloj, self.estado_m1.value, self.estado_m2.value, self.estado_m3.value)
        )
...
def graficar_resultados(resultados: Dict[str, Any]):
    ...
    df_produccion = pd.DataFrame(resultados['cajas_selladas_m3_tiempo'], columns=['Tiempo', 'Cajas Selladas'])
    df_defectos = pd.DataFrame(resultados['defectos_m1_tiempo'], columns=['Tiempo', 'Defectos'])
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_produccion['Tiempo'], y=df_produccion['Cajas Selladas'], name='Cajas Selladas', line=dict(color='green')))
    fig.add_trace(go.Scatter(x=df_defectos['Tiempo'], y=df_defectos['Defectos'], name='Defectos', line=dict(color='red')))
    fig.update_layout(
    ...
    df_tiempos_sistema = pd.DataFrame(resultados['tiempos_sistema_caja'], columns=['Tiempo', 'Tiempo en Sistema'])
    fig_tiempos = go.Figure()
    fig_tiempos.add_trace(go.Scatter(x=df_tiempos_sistema['Tiempo'], y=df_tiempos_sistema['Tiempo en Sistema'], mode='markers', name='Tiempos en Sistema'))
    ...
    df_wip_buffer1 = pd.DataFrame(resultados['wip_buffer1_data'], columns=['Tiempo', 'WIP Buffer 1'])
    df_wip_buffer2 = pd.DataFrame(resultados['wip_buffer2_data'], columns=['Tiempo', 'WIP Buffer 2'])
    
    fig_wip = go.Figure()
    fig_wip.add_trace(go.Scatter(x=df_wip_buffer1['Tiempo'], y=df_wip_buffer1['WIP Buffer 1'], name='WIP Buffer 1', line=dict(color='blue')))
    fig_wip.add_trace(go.Scatter(x=df_wip_buffer2['Tiempo'], y=df_wip_buffer2['WIP Buffer 2'], name='WIP Buffer 2', line=dict(color='orange')))
    ...
    if estados_maquinas:
        df_estados = pd.DataFrame(estados_maquinas, columns=['Tiempo', 'M1', 'M2', 'M3'])
        estado_map = {'OCIOSA': 0, 'PROCESANDO': 1, 'BLOQUEADA': 2, 'INACTIVA_SIN_ENTRADA': 3}
        for m in ['M1', 'M2', 'M3']:
        ...
        for idx, m in enumerate(['M1', 'M2', 'M3']):
            fig_estados.add_trace(go.Scatter(
                ...
                marker=dict(
                    ...
                    colorscale=[[0, 'gray'], [0.33, 'lime'], [0.66, 'red'], [1, 'orange']],
                    cmin=0, cmax=3, size=10, colorbar=dict(title='Estado', tickvals=[0,1,2,3], ticktext=nombres)
                ),
                name=f'Estado {m}',
                text=df_estados[m]
            ))
    ...
    df_items_perdidos = pd.DataFrame(resultados['bloqueos_fuente_tiempo'], columns=['Tiempo', 'Items Perdidos'])
    fig_items_perdidos = go.Figure()
    fig_items_perdidos.add_trace(go.Scatter(x=df_items_perdidos['Tiempo'], y=df_items_perdidos['Items Perdidos'], mode='lines+markers', name='Items Perdidos'))
    ...
    niveles_colas = results.get('niveles_colas_tiempo', [])
    if niveles_colas:
        df_colas = pd.DataFrame(niveles_colas, columns=['Tiempo', 'Cola1', 'Cola2', 'Cola3'])
        fig_colas = go.Figure()
        fig_colas.add_trace(go.Scatter(x=df_colas['Tiempo'], y=df_colas['Cola1'], name='Cola1 (antes M1)', line=dict(color='blue')))
        fig_colas.add_trace(go.Scatter(x=df_colas['Tiempo'], y=df_colas['Cola2'], name='Cola2 (antes M2)', line=dict(color='green')))
        fig_colas.add_trace(go.Scatter(x=df_colas['Tiempo'], y=df_colas['Cola3'], name='Cola3 (antes M3)', line=dict(color='orange')))
    ...
    with col1:
        st.metric("📦 Cajas Selladas (Throughput)", results.get('cajas_selladas_m3', 0))
        st.metric("⚙️ Promedio Tiempo en Sistema por Caja", f"{resultados.get('tiempo_prom_sistema_caja', 0):.2f} min")
        st.metric("📈 Tasa de Producción (Cajas/min)", f"{resultados.get('throughput_cajas_min', 0):.2f}")

    with col2:
        st.metric("❌ Items Perdidos por Buffer1 Lleno", results.get('items_perdidos_buffer1', 0))
        st.metric("⏱️ Tiempo Promedio en Cola 1", f"{resultados.get('acum_tiempo_cola1', 0) / (len(resultados.get('wip_buffer1_data', [])) - 1):.2f} min")
        st.metric("⏱️ Tiempo Promedio en Cola 2", f"{resultados.get('acum_tiempo_cola2', 0) / (len(resultados.get('wip_buffer2_data', [])) - 1):.2f} min")

    # 8. Evolución de bloqueos/fuente bloqueada
    bloqueos_fuente = results.get('bloqueos_fuente_tiempo', [])
    if bloqueos_fuente:
        df_bloqueos = pd.DataFrame(bloqueos_fuente, columns=['Tiempo', 'Items Perdidos Acumulados'])
        fig_bloqueos = go.Figure()
        fig_bloqueos.add_trace(go.Scatter(
